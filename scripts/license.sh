#!/usr/bin/env bash
# PerfGuard -- License Validation Module
# Offline JWT-based license validation (same pattern as APIShield, DepGuard, etc.)
...
PERFGUARD_LICENSE_KEY="${PERFGUARD_LICENSE_KEY:-}"
...
get_perfguard_key() {
  ...
  if [[ -n "$PERFGUARD_LICENSE_KEY" ]]; then
    echo "$PERFGUARD_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('perfguard', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.perfguard?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.perfguard.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
decode_jwt_payload() {
  ...
  # Try multiple decoders for cross-platform support (Linux, macOS, Git Bash)
...
validate_license() {
  ...
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[PerfGuard]${NC} Invalid license key format."
    return 1
  fi
  ...
  if [[ "$iss" != "license-api" ]]; then
    echo -e "${RED}[PerfGuard]${NC} Invalid license issuer."
    return 1
  fi
  ...
  if [[ "$product" != "perfguard" ]]; then
    echo -e "${RED}[PerfGuard]${NC} License is for ${BOLD}$product${NC}, not perfguard."
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[PerfGuard]${NC} License expired."
      echo -e "Renew at: ${CYAN}https://perfguard.pages.dev/renew${NC}"
      return 1
    fi
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[PerfGuard]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://perfguard.pages.dev/upgrade${NC}"
    return 1
  fi
...
# Called by perfguard.sh require_license()

check_perfguard_license() {
  local required_tier="${1:-pro}"

  local key
  if ! key=$(get_perfguard_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- PerfGuard License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://perfguard.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"perfguard\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export PERFGUARD_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
...
get_perfguard_tier() {
  ...
  # Verify it is a perfguard license
  local product
  product=$(extract_field "$payload" "product")
  if [[ "$product" != "perfguard" ]]; then
    echo "free"
...
# Pretty-print license information for perfguard status command

show_perfguard_status() {
  local key
  if ! key=$(get_perfguard_key) || [[ -z "$key" ]]; then
    echo -e "License: ${YELLOW}Free tier${NC}"
    echo -e "  Available: scan (up to 5 source files)"
    echo -e "  Upgrade at: ${CYAN}https://perfguard.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ "$product" != "perfguard" ]]; then
    ...
    echo -e "  This key is for ${BOLD}$product${NC}, not perfguard."
    return
  fi
  ...
  echo -e "    scan (unlimited source files)"
